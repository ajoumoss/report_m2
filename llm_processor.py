...
2. **[6번 ~ 20번 이슈 (완전 신규 발굴 - Zero Tolerance)]**:
   - **[절대 금지]**: 아래 [최근 보고서 이슈 목록]에 언급된 단어나 주제가 **조금이라도 섞여 있다면 무조건 제외**하십시오.
   - **[완전 새로운 주제]**: 지난 3회 보고서에서 단 한 번도 다루지 않은 **완벽하게 새로운 이슈**만 선정해야 합니다.
   - 메인 키워드가 겹치면 아예 쓰지 마십시오. (예: 지난번에 '게임위'가 나왔다면 이번엔 '게임위' 관련 다른 이슈라도 6~20번에는 쓰지 마십시오. 차라리 'e스포츠', '웹툰', '출판' 등 다른 분야를 찾으십시오.)
   - **목표**: 의원님이 "이건 난생 처음 보는 이슈네?"라고 느낄 정도로 **참신한 소식**을 가져와야 합니다.

[최근 보고서 이슈 목록 (이 주제들은 6~20번에서 사용 금지!)]
{"="*30}
...
[기사 선별 및 분석 지침]
1. **TOP 20 선정 전략**: 
   - **1~5번 (핵심 집중)**: **중복 불문.** 가장 논란이 크고 해결되지 않는 '대형 이슈' 5가지를 선정하여 끝까지 물고 늘어지십시오.
   - **6~20번 (신규 발굴)**: **중복 제로(0%).** 위 '히스토리'에 나온 주제는 **티끌만큼도** 겹치지 않게 하십시오. 완전히 새로운 단신, 새로운 지역 축제, 새로운 예술계 소식으로 채우십시오.
