...
- **TOP 20 심층 분석 (10 + 10 방식)**:
  ...
  2. **11 ~ 20번 이슈**: 지난 **24시간 이내**에 보도된 뉴스 중 가장 시급하고 중요한 **최신 현안**을 선정합니다.
     - **중복 방지 필수**: **1~10번에서 다룬 이슈가 11~20번 뉴스에 또 나오더라도 절대 중복해서 선정하지 마십시오.** 이미 다룬 내용은 제외하고, **반드시 다른 새로운 이슈**를 발굴하여 채우십시오. (총 20개의 서로 다른 이슈 필수)
...
위 데이터를 바탕으로 최고의 문체위 현안 질의 참고용 심층 보고서를 작성해주십시오. **20개 이슈 전부 빠짐없이 상세히 작성하는 것을 잊지 마십시오.**
"""
        return prompt
        return prompt